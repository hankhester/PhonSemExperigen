...
//	    formObj.append('<input type="hidden" name="syllNum" value="' + Experigen.screen().syllNum + '">');
//	    formObj.append('<input type="hidden" name="fricative" value="' + Experigen.screen().fric + '">');
//	    formObj.append('<input type="hidden" name="animate" value="' + Experigen.screen()["frame"]["animate"] + '">');
//	    formObj.append('<input type="hidden" name="long" value="' + Experigen.screen().frame.long + '">');
...
Experigen.advance = function(callerButton) {
	...
	var screen = this.screen();
			// uncomment this to show a table of debug info
...
Experigen.eraseLocalData = function () {
	console.log('commented out of experigen file for safety, uncomment at line 898 to erase local data.');
...
Experigen.showDebugInfo = function(screen) {
   var htmlOutput = "";

   var tableStart = '<table id="debugwrapper" align="left" border=2>';
   var tableEnd = "</table>";
   var rowStart = "<tr>";
   var rowEnd = "</tr>";
   var cellStart = '<td valign="top" width="33%">';
   var cellEnd = "</td>";

   var columnHeaders = rowStart
                  + cellStart + 'Settings' + cellEnd
                  + cellStart + 'Current Screen' + cellEnd
                  + cellStart + 'Previous Screen' + cellEnd
                  + rowEnd;

   htmlOutput = htmlOutput + tableStart + columnHeaders;
   htmlOutput = htmlOutput + rowStart;
   htmlOutput = htmlOutput + cellStart + this.delveObject(this.settings, "settings") + cellEnd;
   htmlOutput = htmlOutput + cellStart + this.delveObject(screen, "screen") + cellEnd;

   if (screen.trialnumber > 1) {
      htmlOutput = htmlOutput
            + cellStart
            + this.delveObject(this._screens[screen.trialnumber - 2], "screen")
            + cellEnd;
   }

   htmlOutput = htmlOutput + rowEnd;
   htmlOutput = htmlOutput + tableEnd;
   return htmlOutput;
};

Experigen.delveObject = function(object, subkey) {
   var htmlOutput = "";
   ...
   for (var key in object) {
      if(object.hasOwnProperty(key) && typeof object[key] !== 'function') {
         if (typeof object[key] === 'object') {
            if (subkey) {
               htmlOutput = htmlOutput + Experigen.delveObject(object[key], subkey + "." + key);
            }
            else {
               htmlOutput = htmlOutput + Experigen.delveObject(object[key], key);
            }
         }
         else {
            var keyname = key;
